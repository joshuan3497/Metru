...
if ($_POST && isset($_POST['accion']) && $_POST['accion'] == 'crear_factura') {
    ...
    if ($tipo_cliente == 'existente') {
        ...
        // Si no se seleccionó cliente, usar valores por defecto
        if (!$cliente_id) {
    ...
    foreach ($productos_factura as $producto_data) {
        ...
        if ($producto_id && $cantidad > 0 && $precio > 0) {
            // Verificar que el producto está en la salida
            $producto_en_salida = false;
            foreach ($productos_disponibles as $prod_disp) {
                if ($prod_disp['id'] == $producto_id) {
                    $producto_en_salida = true;
                    break;
                }
            }
            
            if ($producto_en_salida) {
                $productos_validos[] = [
    ...
    if (empty($productos_validos)) {
        $errores[] = "No hay productos válidos en la factura";
    }
    
    // Verificar que no se duplique la factura
    if (empty($errores) && isset($_SESSION['ultima_factura_guardada'])) {
        $tiempo_ultima = $_SESSION['ultima_factura_tiempo'] ?? 0;
        if (time() - $tiempo_ultima < 5) { // 5 segundos de protección
            $_SESSION['mensaje'] = 'Por favor espere antes de crear otra factura';
            $_SESSION['tipo_mensaje'] = 'warning';
            header('Location: mis_facturas.php');
            exit();
        }
    }
    ...
    if (empty($errores)) {
        try {
            ...
            // Crear la factura con TODOS los parámetros
            $factura_id = crearFactura(
                $salida_id, 
                $cliente_id,      // puede ser null para cliente libre
                $usuario_id, 
                $forma_pago, 
                $total_factura, 
                $observaciones,
                $cliente_nombre,  // nombre del cliente libre
                $cliente_ciudad   // ciudad del cliente libre
            );
            ...
            // Guardar información de última factura
            $_SESSION['ultima_factura_guardada'] = $factura_id;
            $_SESSION['ultima_factura_tiempo'] = time();
...
<form method="POST" id="form-factura">
    ...
    <!-- Cliente y Ciudad -->
    <div class="mb-3">
        <label class="form-label fw-bold">Información del Cliente</label>
        
        <!-- Opción 1: Seleccionar cliente existente -->
        <div class="form-check mb-2">
            <input class="form-check-input" type="radio" name="tipo_cliente" id="cliente_existente" value="existente" checked>
            <label class="form-check-label" for="cliente_existente">
                Cliente registrado
            </label>
        </div>
        
        <select class="form-select mb-3" id="cliente_id" name="cliente_id">
        ...
        <!-- Opción 2: Cliente libre -->
        <div class="form-check mb-2">
            <input class="form-check-input" type="radio" name="tipo_cliente" id="cliente_libre" value="libre">
            <label class="form-check-label" for="cliente_libre">
                Cliente no registrado
            </label>
        </div>
        
        <div id="datos_cliente_libre" style="display: none;">
            <input type="text" class="form-control mb-2" name="cliente_nombre" 
    ...
    <script>
    // Toggle entre cliente existente y libre
    $('input[name="tipo_cliente"]').change(function() {
        if ($(this).val() == 'libre') {
            $('#cliente_id').prop('disabled', true).val('');
            $('#datos_cliente_libre').show();
            $('input[name="cliente_nombre"]').prop('required', true);
        } else {
            $('#cliente_id').prop('disabled', false);
            $('#datos_cliente_libre').hide();
            $('input[name="cliente_nombre"]').prop('required', false);
        }
    });
    </script>
